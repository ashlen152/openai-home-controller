...
void loop() {
  ...
  while (networkTask.getResponse(response, 0)) {
    ...
    case NetworkCommand::AUTO_DOSING_RESET:
      if (response.status == NetworkStatus::SUCCESS) {
        Serial.println(
            "[Main] ⏰ Midnight reset detected - resetting auto-dosing");
  ...
  // if (showingSettings && currentTime - lastSettingsDisplayTime >=
  // SETTINGS_DISPLAY_DURATION) {
  //   showingSettings = false;
  //   updateDisplayStatus();
  // }

  // // Calibration result timeout
  // if (showingCalibrationResult && currentTime - lastCalibrationResultTime >=
  // CALIBRATION_RESULT_DURATION) {
  //   showingCalibrationResult = false;
  //   updateDisplayStatus();
  // }

  // // Dosing progress update
  // if (!display.isSleeping() && !inMenu && !showingSettings &&
  // !showingCalibrationResult && currentMode ==
  // DisplayManager::PumpMode::DOSING) {
  ...
  //       elapsedSteps, totalStepsNeeded, pump.isMoving()); if (elapsedSteps >=
  //       totalStepsNeeded || !pump.isMoving()) {
  //         Serial.println("Dosing Complete - Target reached or stopped
  //         moving"); pump.stop(); dosingState =
  //         DisplayManager::DosingState::COMPLETED;
  //         display.showDosingComplete(targetVolume);
  //       } else {
  //         remainingVolume = (totalStepsNeeded - elapsedSteps) /
  //         pump.getStepsPerML(); Serial.printf("Remaining volume: %.2f mL\n",
  //         remainingVolume); display.showDosingProgress(targetVolume,
  //         remainingVolume, wifi.getCurrentTime());
  //       }
  //     } else if ((dosingState == DisplayManager::DosingState::IDLE ||
  //     dosingState == DisplayManager::DosingState::COMPLETED) && !inMenu &&
  //     !showingSettings && !showingCalibrationResult) {
  ...
  //       autoDosing.getRemainingDailyVolume(), currentMode,
  //       wifi.getCurrentTime(), autoDosing.isEnabled(), nextScheduleStr);
  //     }
  //     lastTimeDisplayUpdate = currentTime;
  //   }
  // }
