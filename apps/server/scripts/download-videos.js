...
const CHROME_PORT = parseInt(process.env.CHROME_PORT);

if (!TIKTOK_ACCOUNT) throw new Error('TIKTOK_ACCOUNT env is required');
if (!CHROME_PORT) throw new Error('CHROME_PORT env is required');
...
async function main() {
  ...
  if (args.length < 1) {
    console.log(`Usage: node download-videos.js <urls.json>`);
    console.log(`Example: node download-videos.js tiktok_download/${TIKTOK_ACCOUNT}/urls.json`);
    return;
  }
